...
const InvoicePreview = ({ opened, onClose, invoiceData, isLoading }) => {
  ...
  // 2. Logic: If modal is closed, return null. 
  // We REMOVED "if (!invoice) return null" so the modal can show the Loader.
  ...
  const downloadInvoiceImage = async () => {
    ...
    link.download = `Invoice_${(invoice?.display_name || "Order").replace(/\s+/g, "_")}.png`;
    link.href = image;
    link.click();
  };
    ...
    <Modal
      ...
      styles={{
        content: {
          ...
          minHeight: "400px", // Prevent modal collapse during loading
        },
      }}
    >
      {/* Sticky header */}
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <Button
            ...
            // 3. Disable download while loading
      ...
      >
        {/* 4. Conditional Rendering: Loader vs Content */}
        {isLoading ? (
          <Center h={300}>
            <Loader color="#AB8262" size="lg" type="dots" />
          </Center>
        ) : !invoice ? (
          <Center h={300}>
             <Text c="dimmed">Unable to load invoice details.</Text>
          </Center>
        ) : (
            ...
            >
              {invoice.payment?.payment_status?.toLowerCase() === "paid" ? (
                <>
                  ...
                  <Text>
                    {invoice.payment?.payment_method || "Not provided"}
                  </Text>

                  <Text fw={500}>Status:</Text>
                  <Text>
                    {invoice.payment?.payment_status || "Not provided"}
                  </Text>
